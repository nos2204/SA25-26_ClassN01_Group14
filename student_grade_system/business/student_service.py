...
from persistence.models import (db, StudentModel, GradeModel, SubjectModel,
                                 SemesterModel, AuditLog, DepartmentModel,
                                 UserModel, ClassModel, TeacherModel,
...
class StudentService:
    ...
    def get_dashboard_stats():
        ...
        top5        = (StudentModel.query.filter(StudentModel.gpa > 0)
                       .order_by(StudentModel.gpa.desc()).limit(5).all())
    ...
    def import_grades_from_csv(file_stream, semester_id, actor='admin'):
        ...
        for i, row in enumerate(reader, start=2):
            ...
            if not subject:
                ...
                continue
            StudentService.upsert_grade(student.id, subject.id, semester_id, pg, eg, actor=actor)
            updated += 1
